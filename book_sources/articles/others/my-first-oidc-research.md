...
最後，**refresh token 可以設置成同時間只能存在一個**，因此就算 `client_id`, `client_secret` 都被駭客盜取了，駭客也拿到了 refresh token，使用者也只需要在你的網站上重新登入便可以拿到新的 access token 跟 refresh token，**那麼駭客手上的 refresh token 也就隨之失效了。**
